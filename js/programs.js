...
const ose = (args) => {
    ...
    program.version('1.0.0')
        .description('Ose on your command-line')
        .parse(['', 'ose'].concat(args));
    const ose = require('ose');
    const inquirer = require('inquirer');
    process.stdout.write(ose.bio + '\r\n');
    inquirer.prompt({
        name: 'link',
...
const help = () => {
    process.stdout.write('Available commands: pwd, cd, ls, ose, help');
    process.stdout.emit('line-processed');
};
...
function resolvePath(from, path) {
    ...
    if (path.startsWith('/')) {
        ...
        absPath = 'warchief' + path.substring(1 /* '~'.length */);
    } else {
