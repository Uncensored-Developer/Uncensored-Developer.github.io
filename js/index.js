...
const PROMPT = () => {
    ...
    window.rawPrompt = `warchief @ ${pwd} ${PROMPT_CHAR} `;
    return `${chalk[COLOURS.user]('warchief')} @ ${chalk[COLOURS.pwd](pwd)} ${PROMPT_CHAR} `;
};
...
function setUpTermEventHandlers() {
    ...
    term.on('newline', (line) => {
        const recognisedCommands = ['ose', 'help', 'ls', 'pwd', 'cd', 'exit', 'rm'];
...
function startTerminalSession() {
    term.writeln(chalk[COLOURS.command]('npm') + ' install -g git+https://github.com/Uncensored-Developer/ose.git');
